WITH new_job AS (
  ...
  SELECT
    source, 'user13-synthetic-demo-20260923', title, company, location,
    description || ' [DEMO TEST JOB 2026-09-23: synthetic, cloned from job 593 for manager demo. Not a real posting.]',
    url, is_remote, min_experience_years, max_experience_years, now(),
    encode(sha256(convert_to('user13-synthetic-demo-' || clock_timestamp()::text, 'UTF8')), 'hex'),
    true, embedding, now(),
